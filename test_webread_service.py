...
def test_webread_service_returns_text_and_then_cache_hit(monkeypatch, tmp_path) -> None:
    ...
    def fake_extract(url: str, *, instruction=None, **_kwargs):
    ...
    monkeypatch.setattr(webread_service, "extract_clean_text", fake_extract)
...
def test_extract_returns_l3_busy_when_lock_times_out(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(webread_service, "extract_clean_text", fail_extract)
...
def test_extract_releases_lock_after_success(monkeypatch, tmp_path) -> None:
    ...
    def fake_extract(url: str, *, instruction=None, allow_l3=None, **_kwargs):
        assert url == "https://example.com"
        assert instruction is None
        assert allow_l3 is True
...
def test_extract_non_l3_path_skips_lock_and_passes_allow_l3_false(monkeypatch, tmp_path) -> None:
    ...
    def fake_extract(url: str, *, instruction=None, allow_l3=None, **_kwargs):
        extract_calls.append((url, instruction, allow_l3))
        return {
            "url": url,
            "domain": "example.com",
            "title": "Example Domain",
    ...
    assert extract_calls == [
        ("https://example.com/1", None, False),
        ("https://example.com/2", None, False),
    ]
